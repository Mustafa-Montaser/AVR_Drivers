...
//	SET_BIT(IIC_TWCR, IIC_TWCR_TWINT);
//
//	/* Waiting till Job Is Done [Stop Condition] */
//	while(GET_BIT(IIC_TWCR, IIC_TWCR_TWINT) == 0);
//
//}
...
//#if IIC_OPERATION_MODE == IIC_SLAVE_MODE
//
//u8 IIC_u8SlaveListen(void) {
//	u8 local_u8Status;
//	u8 local_u8ReturnState;
//	 while(1) {
//		while(GET_BIT(IIC_TWCR, IIC_TWCR_TWINT) == 0);
//		local_u8Status = IIC_TWSR & 0xF8;
//		if(local_u8Status == 0x60||local_u8Status == 0x68)	/* Own SLA+W received &ack returned */
//			local_u8ReturnState = 1;			/* Return 0 to indicate ack returned */
//		else if(local_u8Status == 0xA8||local_u8Status == 0xB0)	/* Own SLA+R received &ack returned */
//			local_u8ReturnState = 2;			/* Return 0 to indicate ack returned */
//		else if(local_u8Status == 0x70||local_u8Status == 0x78)	/* General call received &ack returned */
//			local_u8ReturnState = 3;			/* Return 1 to indicate ack returned */
//	 }
